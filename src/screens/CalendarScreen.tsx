...
export default function CalendarScreen() {
  ...
  const daysOfWeek = [ { id: 1, name: 'Lunes' }, { id: 2, name: 'Martes' }, { id: 3, name: 'Miércoles' }, { id: 4, name: 'Jueves' }, { id: 5, name: 'Viernes' }, { id: 6, name: 'Sábado' }, { id: 7, name: 'Domingo' } ];

  useFocusEffect(useCallback(() => { loadCalendar(); }, []));
  ...
  const handleCardPress = (rule: Rule) => { setSelectedRule(rule); setModalVisible(true); };

  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}><Text style={styles.title}>CALENDARIO SEMANAL</Text><Text style={styles.subtitle}>Planifica tu ahorro</Text></View>
      {loading ? ( <ActivityIndicator size="large" color="#D4AF37" style={{ marginTop: 50 }} /> ) : (
        <ScrollView style={styles.scroll}>
          {daysOfWeek.map((day) => {
            const rulesForDay = weeklyRules.filter(r => r.days.includes(day.id) && !hiddenBanks.includes(r.issuer_id));
            if (rulesForDay.length === 0) return null;
            return (
              <View key={day.id} style={styles.daySection}>
                <View style={styles.dayHeaderRow}><Text style={styles.dayTitle}>{day.name}</Text><View style={styles.dayLine} /></View>
                {rulesForDay.map((rule, index) => {
                  ...
                  return (
                    <TouchableOpacity key={index} activeOpacity={0.9} onPress={() => handleCardPress(rule)}>
                      <View style={[styles.card, { backgroundColor: bgColor }]}>
                        <View style={styles.cardHeader}>
                          ...
                          <View style={styles.discountBadge}><Text style={styles.discountText}>{rule.benefit_value}</Text></View>
                        </View>
                        ...
                        <View style={styles.tipContainer}><Text style={styles.tipIcon}>💡</Text><Text style={styles.cardTip} numberOfLines={1}>{rule.smart_tip}</Text></View>
                      </View>
                    </TouchableOpacity>
                  );
                })}
      ...
      <RuleDetailModal isVisible={modalVisible} onClose={() => setModalVisible(false)} rule={selectedRule} bank={selectedRule ? getBank(selectedRule.issuer_id) : undefined} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 16, padding: 16, marginBottom: 12, elevation: 5 },
