...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}><Text style={styles.greeting}>AL DÍA</Text><Text style={styles.subGreeting}>Tu Estrategia de Hoy</Text></View>
      <ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => {setRefreshing(true); loadData();}} tintColor="#D4AF37" />}>
        <Text style={styles.sectionTitle}>OPORTUNIDADES DE HOY</Text>
        {loading ? ( <ActivityIndicator size="large" color="#D4AF37" style={{marginTop: 50}} /> ) : rules.length > 0 ? (
            ...
            return (
              <TouchableOpacity key={index} activeOpacity={0.9} onPress={() => handleCardPress(item)}>
                <View style={[styles.card, { backgroundColor: bgColor }]}>
                  ...
                  <View style={styles.tipContainer}><Text style={styles.tipIcon}>💡</Text><Text style={styles.cardTip} numberOfLines={1}>{item.smart_tip}</Text></View>
                </View>
              </TouchableOpacity>
            );
          })
        ) : ( <View style={{alignItems: 'center', marginTop: 50}}><Text style={{fontSize: 40}}>💤</Text><Text style={styles.emptyText}>No hay alertas activas.</Text></View> )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 28, fontWeight: '800', color: '#D4AF37' },
  subGreeting: { fontSize: 14, color: '#666', marginTop: 4, textTransform: 'uppercase' },
  scrollContent: { padding: 20 },
  sectionTitle: { color: '#FFFFFF', fontSize: 14, marginBottom: 15, fontWeight: '700', opacity: 0.5 },
